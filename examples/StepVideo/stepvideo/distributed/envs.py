...
class PackagesEnvChecker:
    ...
    def check_aiter(self):
        ...
        try:
            ...
            logging.info("Using AITER as the attention library")
            return True
        except:
            if _is_hip():
                logging.warning(
    ...
    def check_flash_attn(self, packages_info):
        ...
        if _is_npu():
            logging.info("falsh_attn is not ready on torch_npu for now")
            return False

        if _is_musa():
            logging.info("Flash Attention library is not supported on MUSA for the moment.")
            return False
        try:
    ...
    def check_long_ctx_attn(self):
        ...
        try:
            ...
            logging.warning(
                'Ring Flash Attention library "yunchang" not found, using pytorch attention implementation'
            )
            return False
