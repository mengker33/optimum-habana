...
class StepVideoPipeline(DiffusionPipeline):
    ...
    def decode_vae(self, samples):
        samples = asyncio.run(self.vae(samples))
        return samples
    ...
    def prepare_latents(
        ...
        latents = torch.randn(shape, generator=generator, device="cpu", dtype=dtype).to(self.device)
        return latents
    ...
    def __call__(
        ...
        if task == "ti2v":
            ...
            # In t2v case, no ref img for condition_hidden_states
