...
from models_gpt.models.gpt2_rope2_time_new_correct_mask_noncasual_reflow import GPT2Model, precompute_freqs_cis
...
class BASECFM(torch.nn.Module, ABC):
    def __init__(
        ...
        self.estimator = wrap_in_hpu_graph(estimator)
    ...
    def solve_euler(self, x, latent_mask_input,incontext_x, incontext_length, t_span, mu,attention_mask, guidance_scale):
        ...
        for i in tqdm(range(len(dt))):
            ti = t[0]
            t = torch.roll(t, shifts=-1, dims=0)
            ...
            head_dim = self.estimator.embed_dim // 20
            freqs_cis = precompute_freqs_cis(dim=head_dim, end=model_input.size(1))
            cos, sin = torch.view_as_real(freqs_cis).to(model_input.device).unbind(-1)
            ...
            v = self.estimator(inputs_embeds=model_input,
                            ...
                            time_step=timestep, freqs_cis=(cos, sin)).last_hidden_state
            v = v[..., -x.shape[2]:]
            ...
            x_next = x_next + dt[0] * v
            dt = torch.roll(dt, shifts=-1, dims=0)
