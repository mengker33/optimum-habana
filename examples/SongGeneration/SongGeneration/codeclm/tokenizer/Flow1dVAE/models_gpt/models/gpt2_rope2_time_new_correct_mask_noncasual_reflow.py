...
class GPT2Attention(nn.Module):
    ...
    def _attn(self, query, key, value, freqs_cis, attention_mask=None, head_mask=None):
        
        query = query.transpose(1, 2)
        query = apply_rotary_pos_emb(query, *freqs_cis, None, 0, RotaryPosEmbeddingMode.PAIRWISE)

        query = query.transpose(1, 2)
        if query.shape == key.shape:
            key = key.transpose(1, 2)
            key = apply_rotary_pos_emb(key, *freqs_cis, None, 0, RotaryPosEmbeddingMode.PAIRWISE)
            key = key.transpose(1, 2)
    ...
    def forward(
        ...
        freqs_cis: Optional[Tuple[torch.Tensor]] = None,
        ...
        if self.reorder_and_upcast_attn:
            ...
            attn_output, attn_weights = self._attn(query, key, value, freqs_cis, attention_mask, head_mask)
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        freqs_cis: Optional[torch.LongTensor] = None,
