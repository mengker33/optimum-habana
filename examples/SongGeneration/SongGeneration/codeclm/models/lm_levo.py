...
class LmModel(StreamingModule):
    ...
    def __init__(self, 
        ...
        self.transformer2 = wrap_in_hpu_graph(self.transformer2)
    ...
    def forward(self, 
                ...
                position_ids = None,
                attn_mask = None,
                gen_sequence_len = 0) -> torch.Tensor:
        ...
        seq_len = fused_input1.shape[1]
        if seq_len > 1:
            total_len = math.ceil((seq_len + gen_sequence_len) / 512) * 512
            pad_len = total_len - seq_len
            attn_mask = torch.ones((B, seq_len), device=fused_input1.device).to(torch.bool)
            fused_input1 = F.pad(fused_input1, (0, 0, 0, pad_len, 0, 0), value=0)
            fused_input2 = F.pad(fused_input2, (0, 0, 0, pad_len, 0, 0), value=0)
            attn_mask = F.pad(attn_mask, (0, pad_len, 0, 0), value=False)
            token_idx = torch.tensor([seq_len], dtype=torch.long, device=fused_input1.device)
            cache_idx = None
        elif seq_len == 1:
            token_idx = token_idx + 1
            cache_idx = math.ceil(int(token_idx) / 256) * 256
        ...
        if seq_len > 1:
            position_ids = torch.tensor([seq_len], dtype=torch.long, device=fused_input1.device).unsqueeze(0)
        elif seq_len == 1:
            position_ids = position_ids + 1
            attn_mask=attn_mask.index_fill_(1, token_idx, 1)

        # remove the prefix from the model outputs
        if len(self.fuser.fuse2cond['prepend']) > 0:
            logits = logits[:, :, seq_len-S:seq_len, :]

        return logits, token_idx, position_ids, attn_mask
    ...
    def generate(self, #
        ...
        ignore_tokens = ignore_tokens[ignore_tokens < 16384].to('hpu')
        ...
        with self.streaming():
            ...
            for offset in tqdm(range(start_offset_sequence, gen_sequence_len)):
                ...
                htorch.core.mark_step()
                next_token, token_idx, position_ids, attn_mask = self._sample_next_token(
                    curr_sequence.to('hpu'), condition_tensors, use_sampling, temp, top_k, top_p,
                    cfg_coef=cfg_coef, 
                    ...
                    position_ids = position_ids,
                    attn_mask = attn_mask,
                    gen_sequence_len = gen_sequence_len,
                    )
    ...
    def _sample_next_token(self,
        ...
        htorch.core.mark_step()
        all_logits, token_idx, position_ids, attn_mask = \
            model(sequence, condition_tensors=condition_tensors,
                           token_idx = token_idx,
                           position_ids = position_ids,
                           attn_mask = attn_mask,
                           gen_sequence_len = gen_sequence_len)
        ...
        return next_token, token_idx, position_ids, attn_mask
