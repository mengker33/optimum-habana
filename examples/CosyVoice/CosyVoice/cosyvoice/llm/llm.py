...
class Qwen2Encoder(torch.nn.Module):
    ...
    def forward_one_step(self, xs, masks, cache=None, token_idx=None, position_ids=None):
        input_masks = masks[:, -1, :]
        outs = self.model(
            inputs_embeds=xs,
...
#        print(" xs shape: " + str(xs.shape) + " new_cache shape: " + str(new_cache[0][0].shape))
        return xs, new_cache
...
class Qwen2LM(TransformerLM):
    ...
    def inference(
        ...
        device = 'hpu' #text.device
        text = torch.concat([prompt_text, text], dim=1)
        text_len += prompt_text_len
        text = self.llm.model.model.embed_tokens(text)

        # 3. concat llm_input
        sos_eos_emb = self.llm_embedding.weight[self.sos_eos].reshape(1, 1, -1).to(device)
        task_id_emb = self.llm_embedding.weight[self.task_id].reshape(1, 1, -1).to(device)
        if prompt_speech_token_len != 0:
            prompt_speech_token_emb = self.speech_embedding(prompt_speech_token)
        else:
            prompt_speech_token_emb = torch.zeros(1, 0, self.llm_input_size, dtype=text.dtype).to(device)
        lm_input = torch.concat([sos_eos_emb, text, task_id_emb, prompt_speech_token_emb], dim=1).to(device).bfloat16()
    ...
    def inference_wrapper(self, lm_input, sampling, min_len, max_len, uuid):
        if hasattr(self, 'vllm'):
            ...
            mask = F.pad(mask, (0, pad_len, 0, 0, 0, 0), value=False)
            for i in range(max_len):
                ...
                mask=mask.index_fill_(2, token_idx, 1)
                y_pred = y_pred[:, prompt_len + i - 1] if i == 0 else y_pred[:, -1]
                logp = self.llm_decoder(y_pred).float().log_softmax(dim=-1).to('cpu')
                top_ids = self.sampling_ids(logp.squeeze(dim=0), out_tokens, sampling, ignore_eos=True if i < min_len else False).item()
                if top_ids == self.speech_token_size:
                    break
                if top_ids > self.speech_token_size:
                ...
                yield top_ids
                out_tokens.append(top_ids)
                lm_input = self.speech_embedding.weight[top_ids].reshape(1, 1, -1).to('hpu').bfloat16()
