...
class CosyVoiceModel:
        ...
        with self.llm_context:
            if isinstance(text, Generator):
                ...
                for i in self.llm.inference(text=text.to('hpu'), #.to(self.device),
                                            ...
                                            prompt_text=prompt_text.to('hpu'), #.to(self.device),
                                            prompt_text_len=torch.tensor([prompt_text.shape[1]], dtype=torch.int32).to(self.device),
                                            prompt_speech_token=llm_prompt_speech_token.to('hpu'), #.to(self.device),
                                            prompt_speech_token_len=torch.tensor([llm_prompt_speech_token.shape[1]], dtype=torch.int32).to(self.device),
    ...
    def token2wav(self, token, prompt_token, prompt_feat, embedding, uuid, finalize=False, speed=1.0):
        ...
        tts_mel, self.flow_cache_dict[uuid] = self.flow(token=token.to(self.device),
...
class CosyVoice2Model(CosyVoiceModel):
    ...
    def token2wav(self, token, prompt_token, prompt_feat, embedding, token_offset, uuid, stream=False, finalize=False, speed=1.0):
        ...
        tts_mel, _ = self.flow(token=token.to(self.device),
                                          ...
                                          finalize=finalize)
        t2 = time.time()
        tts_mel = tts_mel[:, :, token_offset * self.flow.token_mel_ratio:].cpu()
        # append hift cache
        if self.hift_cache_dict[uuid] is not None:
            hift_cache_mel, hift_cache_source = self.hift_cache_dict[uuid]['mel'], self.hift_cache_dict[uuid]['source']
            tts_mel = torch.concat([hift_cache_mel, tts_mel], dim=2)
        ...
        # keep overlap mel and hift cache
        if finalize is False:
            tts_speech, tts_source = self.hift(speech_feat=tts_mel.to('cpu'), cache_source=hift_cache_source)
            tts_speech = tts_speech.cpu()
            tts_source = tts_source.cpu()
            if self.hift_cache_dict[uuid] is not None:
                tts_speech = fade_in_out(tts_speech, self.hift_cache_dict[uuid]['speech'], self.speech_window)
            self.hift_cache_dict[uuid] = {'mel': tts_mel[:, :, -self.mel_cache_len:],
                                          'source': tts_source[:, :, -self.source_cache_len:],
            ...
            if self.hift_cache_dict[uuid] is not None:
                tts_speech = fade_in_out(tts_speech, self.hift_cache_dict[uuid]['speech'], self.speech_window)
        t4 = time.time()
        # print("*********** self.flow latency: " + str(t2-t1) + " sec")
        # print("*********** concat    latency: " + str(t3-t2) + " sec")
        # print("*********** self.hit  latency: " + str(t4-t3) + " sec")
        return tts_speech
