...
def main():
    ...
    if args.batch:
        test_dataset = Dataset(args.prompt_data, data_pipeline=configs['data_pipeline'], mode='train', shuffle=True, partition=False)
    ...
    with torch.no_grad():
        for _, batch in tqdm(enumerate(test_data_loader)):
            utts = batch["utts"]
            logging.info(f"inference batch size: {len(utts)}")
            ...
            if args.batch:
                for model_output in model.batch_inference(**model_input):
                    music_audios.append(model_output['music_audio'])
                    text_prompts.append(model_output['text'])
            else:
            ...
            for idx, music_audio in enumerate(music_audios):
                music_key = utts[idx]
                music_fn = os.path.join(args.result_dir, f'{music_key}.{args.format}')
                if args.trim:
                    music_audio = trim_audio(music_audio, sample_rate=args.output_sample_rate, threshold=0.05, min_silence_duration=0.8)
                ...
                if music_audio.shape[0] != 0:
                    ...
                    if music_audio.shape[1] >= min_generate_audio_length:
                        try:
                            ...
                            if music_audio.dtype == torch.float16:
                                music_audio = torch.tensor(music_audio, dtype=torch.float32)
                        ...
                        if args.batch:
                            ...
                            logging.info(f"processing time: {int(bench_end - bench_start)}s, batch size: {len(text_prompts)}, key: {music_key}, rtf: {rtf/len(text_prompts)}, text prompt: {text_prompt}")
                            caption_f.write('{}\t{}\n'.format(music_key, text_prompt))
                            caption_f.flush()
                        else:
                            logging.info(f"processing time: {int(bench_end - bench_start)}s, audio length: {int(audio_duration)}s, key: {music_key}, rtf: {rtf}, text prompt: {text_prompt}")
                            caption_f.write('{}\t{}\n'.format(music_key, text_prompt))
                            caption_f.flush()
                    else:
